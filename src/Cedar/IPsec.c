...
void IPsecProcPacket(IPSEC_SERVER *s, UDPPACKET *p)
{
	...
	if (proc_this_packet)
	{
		...
		case IPSEC_PORT_IPSEC_ESP_RAW: {
			...
			default: {
				Debug("Unknown UDP packet type: %u, no process\n", p->Type);
				break;
			}
			}
		}
...
void IPsecServerUdpPacketRecvProc(UDPLISTENER *u, LIST *packet_list)
{
	...
	/*if (ikev2->Interrupts == NULL)
	{
		ikev2->Interrupts = u->Interrupts;
	}*/
	...
	// Interrupt processing of IKE server (ONLY IKEv1)
	...
	for (i = 0; i < LIST_NUM(ikev2->SendPacketList); ++i)
	{
		...
		{
			// START RECURSION
			IKEv2_CRYPTO_PARAM* prm = LIST_DATA(s->Ikev2->ipsec_SAs, 0);
			prm->key_data->sk_ai = prm->key_data->sk_ar;
	...
	UdpListenerSendPackets(u, ikev2->SendPacketList);

	DeleteAll(ike->SendPacketList);
	DeleteAll(ikev2->SendPacketList);
}
...
void IPsecServerSetServices(IPSEC_SERVER *s, IPSEC_SERVICES *sl)
{
	...
	Lock(s->LockSettings);
	{
		...
		if (IsEmptyStr(sl->IPsec_Secret) == false)
		{
			...
			StrCpy(s->Ikev2->ike_server->Secret, sizeof(s->Ikev2->ike_server->Secret), sl->IPsec_Secret);
		}
